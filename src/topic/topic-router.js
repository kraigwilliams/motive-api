...
topicRouter.route("/:topicId/thoughts").get(async (req, res, next) => {
  const knexInstance = req.app.get("db");
  const userId = req.user.id;

  try {
    const thoughts = await TopicService.getAllThoughts(
      knexInstance,
      req.params.topicId
    );
  ...
  .get(async (req, res, next) => {
    ...
    try {
      if (req.params.sort_option == 2) {
        const alphaThoughts = await TopicService.getSortedThoughts(
          knexInstance,
          req.params.topicId
        );
...
topicRouter.route("/shared").get(async (req, res, next) => {
  ...
  try {
    ...
    console.log(
      "get shared topics by user id error start",
      error,
      "get shared topics by user id error end"
    );
    next(error);
  }
});
...
topicRouter
  ...
  .get(async (req, res, next) => {
    try {
      ...
      console.log(
        "get topic by id error start",
        error,
        "get topic by id error end"
      );
      next(error);
    }
  })
  ...
  .delete(async (req, res, next) => {
    try {
      ...
      console.log(
        "delete thought error start",
        error,
        "delete thought error end"
      );
      next(error);
    }
  });

topicRouter.route("/:topicId/level").get(async (req, res, next) => {
  const knexInstance = req.app.get("db");
  ...
  try {
    ...
    if (!level) {
      return res.status(404).json({
        error: {
          message: `Error when finding shared topic in topic_connections.`,
        },
      });
    }
    res.json(level);
...
topicRouter
  ...
  .post(jsonBodyParser, async (req, res, next) => {
    ...
    try {
      ...
      TopicService.shareTopic(knexInstance, sharedTopic).then(
        async (topicShared) => {
          ...
          const thoughtsInTopic = await TopicService.getAllThoughts(
            knexInstance,
            topicId
          );
          ...
          thoughtsInTopic.map((thought) => {
            ...
            TopicService.insertSharedTopicThoughts(
              knexInstance,
              thoughtToInsert
            );
          });
      ...
      res.status(201).json(sharedTopic);
    } catch (error) {
      console.log(
        "topic router error sharing topic start",
        error,
        "end of topic sharing error"
      );
      next(error);
    }
  });
