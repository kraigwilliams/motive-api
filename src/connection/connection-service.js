    ...
    return knex
      ...
      .whereNotExists(function(){
        this.select('*').from('connections')
          .whereRaw( '?? = ??', ['connections.sender_id', 'fu.id'])
          .orWhereRaw('?? = ??', ['connections.receiver_id', userId])
          .orWhereRaw('?? = ??', ['connections.receiver_id', 'fu.id'])
          .orWhereRaw( '?? = ??',['connections.sender_id', userId]);
      }); 
  }
};
